...
namespace ToolKits
{
    ...
    public static class PreviewWindowEditorHelper
    {
    ...
    public class PreviewWindow : EditorWindow
    {
        ...
        private void Init()
        {
            _previewPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(AVATAR_PATH);
            _animationClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(CLIP_PATH);
            _motion = _animationClip;
        }
        ...
        private void OnGUI()
        {
            ...
            _motion = (Motion)EditorGUILayout.ObjectField("Motion", _motion, typeof(Motion), false);

            if (GUILayout.Button("加载预览"))
            {
                if (_previewPrefab == null || _motion == null)
                {
                    EditorUtility.DisplayDialog("缺少输入", "请先设置 Prefab 和 Motion。", "OK");
                    return;
                }
                ...
                PreviewInstance = PreviewWindowEditorHelper.InstantiateGoByPrefab(_previewPrefab, null);
                PreviewInstance.hideFlags = HideFlags.HideAndDontSave;
                TrySetPreviewTag(PreviewInstance, PreviewWindowConstants.PREVIRE_TAG);
                _previewAnimator = AssetDatabase.LoadAssetAtPath<AnimatorController>(PREVIEW_ANIMCONTROLLER_PATH);
                if (_previewAnimator == null)
                {
                    _previewAnimator = new AnimatorController();
                    _previewAnimator.AddLayer("Base Layer");
                }

                if (_previewAnimator.layers == null || _previewAnimator.layers.Length == 0 || _previewAnimator.layers[0].stateMachine == null)
                {
                    EditorUtility.DisplayDialog("控制器无效", "Preview AnimatorController 缺失或无可用层。", "OK");
                    return;
                }
                ...
                {
                    if (states[i].state != null && states[i].state.name == "Preview")
                    {
                        _animatorState = states[i].state;
                        break;
                    }
                }
                if (_animatorState == null)
                {
                    _animatorState = _previewAnimator.layers[0].stateMachine.AddState("Preview");
                }

                InitController();

                _avatarPreview = new AvatarPreview(_animator, _motion);
                _avatarPreview.OnAvatarChangeFunc = SetPreviewAvatar;
                if (_motion is AnimationClip clip)
                {
                    _animationClip = clip;
                    _avatarPreview.fps = Mathf.RoundToInt(_animationClip.frameRate);
                    _avatarPreview.timeControl.stopTime = _animationClip.length;
                    _avatarPreview.ShowIKOnFeetButton = clip.isHumanMotion;
                }
                else
                {
                    _animationClip = null;
                    _avatarPreview.ShowIKOnFeetButton = false;
                }
            ...
            if (null != _avatarPreview)
            {
                ...
                _avatarPreview.DoAvatarPreview(PREVIEW_RECT, PreviewWindowConstants.preBackgroundSolid);
            }
        ...
        private void InitController()
        {
            ...
            if (_animator == null)
            {
                _animator = PreviewInstance.AddComponent<Animator>();
            }
            _animatorState.motion = _motion;
        ...
        private static bool HasPreviewTag()
        {
            ...
            {
                if (tags[i] == PreviewWindowConstants.PREVIRE_TAG)
                {
                    return true;
                }
            }
...
namespace ToolKits
{
    ...
    {
        public static readonly UnityEngine.GUIStyle preBackgroundSolid = PreviewWindowConstants.preBackgroundSolid;
        public const string PREVIRE_TAG = PreviewWindowConstants.PREVIRE_TAG;
    }

    public static class EditorHelper
    {
        public static UnityEngine.GameObject InstantiateGoByPrefab(UnityEngine.GameObject prefab, UnityEngine.GameObject parent)
        {
            return PreviewWindowEditorHelper.InstantiateGoByPrefab(prefab, parent);
        }
    }
}
namespace ToolKits
...
namespace ToolKits
{
    public class ReflectionHelper
    {
        ...
        public static ReflectionHelper Instance => instance ?? (instance = new ReflectionHelper());
    ...
    {
        private readonly PropertyInfo bodyPositionInternal;

        public ReflectAnimator()
        {
            bodyPositionInternal = typeof(Animator).GetProperty("bodyPositionInternal", BindingFlags.NonPublic | BindingFlags.Instance);
        }

        public Vector3 GetBodyPosition(Animator animator)
        {
            if (bodyPositionInternal != null)
            {
                return (Vector3)bodyPositionInternal.GetValue(animator, null);
            }
    ...
    {
        private readonly MethodInfo applyWireMaterial;

        public RefHandleUtility()
        {
            applyWireMaterial = typeof(HandleUtility).GetMethod("ApplyWireMaterial", Type.EmptyTypes);
        }
    ...
    {
        private readonly MethodInfo getAnimationClipsFlattened;

        public RefBlendTree()
        {
            getAnimationClipsFlattened = typeof(UnityEditor.Animations.BlendTree).GetMethod("GetAnimationClipsFlattened", BindingFlags.Instance | BindingFlags.NonPublic);
        }

        public AnimationClip[] GetAnimationClipsFlattened(UnityEditor.Animations.BlendTree blendTree)
        {
            if (getAnimationClipsFlattened != null)
            {
                return getAnimationClipsFlattened.Invoke(blendTree, null) as AnimationClip[];
            }
    ...
    {
        private readonly MethodInfo calculateBestFittingPreviewGameObject;

        public RefModelImporter()
        {
            calculateBestFittingPreviewGameObject = typeof(ModelImporter).GetMethod("CalculateBestFittingPreviewGameObject", BindingFlags.Instance | BindingFlags.NonPublic);
        }

        public string CalculateBestFittingPreviewGameObject()
        {
            if (calculateBestFittingPreviewGameObject != null)
            {
                return calculateBestFittingPreviewGameObject.Invoke(null, null) as string;
            }
    ...
    {
        private const int PreviewCullingLayerFallback = 31;
        private readonly FieldInfo previewCullingLayerField;

        public RefCamera()
        {
            previewCullingLayerField = typeof(Camera).GetField("PreviewCullingLayer", BindingFlags.Static | BindingFlags.NonPublic);
        }
        ...
        public int PreviewCullingLayer
        {
            get
            {
                if (previewCullingLayerField != null)
                {
                    return (int)previewCullingLayerField.GetValue(null);
                }

                return PreviewCullingLayerFallback;
            }
        }
    }
    ...
    {
        private readonly MethodInfo instantiateForAnimatorPreview;

        public RefEditorUtility()
        {
            instantiateForAnimatorPreview = typeof(EditorUtility).GetMethod("InstantiateForAnimatorPreview", BindingFlags.Static | BindingFlags.NonPublic);
        }

        public GameObject InstantiateForAnimatorPreview(Object original)
        {
            if (instantiateForAnimatorPreview != null)
            {
                return instantiateForAnimatorPreview.Invoke(null, new[] { original }) as GameObject;
            }
    ...
    {
        private readonly MethodInfo setPreview;

        public RefAvatarPreviewSelection()
        {
            Type t = Type.GetType("UnityEditor.AvatarPreviewSelection,UnityEditor");
            if (t != null)
            {
                setPreview = t.GetMethod("SetPreview", BindingFlags.Static | BindingFlags.Public);
            }
        }

        public void SetPreview(ModelImporterAnimationType type, GameObject go)
        {
            setPreview?.Invoke(null, new object[] { type, go });
        }
    }
}
...
namespace ToolKits
{
    public class AvatarPreview
    {
            ...
            ModelImporterAnimationType animationType)
        {
            ...
            TrySetPreviewTag(selected, Constants.PREVIRE_TAG);
        ...
        void SetupBounds(GameObject go)
        {
            ...
            if (go != null)
            {
                ...
                TrySetPreviewTag(m_PreviewInstance, Constants.PREVIRE_TAG);
        ...
        void InitInstance(Animator scenePreviewObject, Motion motion)
        {
            ...
            if (m_ReferenceInstance == null)
            {
                ...
                TrySetPreviewTag(m_ReferenceInstance, Constants.PREVIRE_TAG);
            ...
            if (m_DirectionInstance == null)
            {
                ...
                TrySetPreviewTag(m_DirectionInstance, Constants.PREVIRE_TAG);
            ...
            if (m_PivotInstance == null)
            {
                ...
                TrySetPreviewTag(m_PivotInstance, Constants.PREVIRE_TAG);
            ...
            if (m_RootInstance == null)
            {
                ...
                TrySetPreviewTag(m_RootInstance, Constants.PREVIRE_TAG);
        ...
        private static bool HasPreviewTag()
        {
            ...
            for (int i = 0; i < tags.Length; i++)
            {
                if (tags[i] == Constants.PREVIRE_TAG)
                {
                    return true;
                }
            }
